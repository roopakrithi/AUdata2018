...
# vars <- c(1, 20, 8, 9, 11, 12, 13, 15, 16, 17, 18, 19, 22, 25, 26, 18, 68, 91)
...
par(mfrow = c(1,1))
# travel time by village
boxplot(au$travel.time ~ au$villageID)

# area by village
boxplot(au$AU.area ~ au$villageID)
boxplot(au$AU.hh ~ au$villageID)
...
par(mfrow = c(1, 1))
for(i in 1:7){
  au2 <- subset(au, villageID == 2 & type == "active")
  hist(au2[, 11], main = i, , col = rgb(0, 0, 1, .5))
}

for(i in 1:7){
  au2 <- subset(au, villageID == 2 & (type == "underused" | type == "unused" ))
  hist(au2[, 11], main = i, col = rgb(1, 0, 0, .5), add = T)
}

for(i in 1:7){
  au2 <- subset(au, villageID == i, breaks = 10)
  hist(au2[, 20], add = T)
}

#for(i in 1:7){
  # stacked bar plots of max threat, by count and area}

par(mfrow = c(1,1))
barplot(table(au$max.threat, au$village), col=c("black", "lightgrey", "yellow", "orange", "red", "darkred") , border="white", space=0.04, font.axis=2, xlab="village")

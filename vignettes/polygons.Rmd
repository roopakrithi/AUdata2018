...
for(i in 1:3){
  reg <- au[au$region == i, ]
  plot(reg)
 plot(subset(reg, maj.active == 0), col = "darkred", add = T)
 plot(subset(reg, maj.active == 1), col = "lightgreen", add = T)
 
 box(lty = 1, col = "grey")
  mtext("Majority of area is actively cultivated", side = 3, outer = TRUE)
  if ( i == 1){
  legend("bottomright", c("yes", "no"), fill = c("lightgreen", "darkred"), 
         bty = "n")
  }
}
...
for(i in 1:3){
 ...
 box(lty = 1, col = "grey")
  mtext("pc cultivated", side = 3, outer = TRUE)
  if ( i == 1){

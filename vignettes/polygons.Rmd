...
### NEED TO FIX ***IMPORTANT: ANYTHING THAT'S IN TWO PARTS, OLD AND NEW*****
# bada phat
# kusmdin
# kathyaan
## go directly into the csv and change these. just edit the original version to reflect 
  #what's happening in the whole. KEEP the old one as is. In notes, make sure to note that it's a subpart.

### THEN, do this again:
# aggregate
# subset out just IDlong
# merge with corrected dat
# write to package (as au2)
# push all 
# read in au2 (as au) and proceed as before

## make polygons with same IDlong into multipart polygons
au <- aggregate(au, by = list(au$IDlong), FUN = min()) ## BE CAREFUL. I THINK THIS WORKED BUT... CHECK.
...
writeOGR(au2, 
        ...
        layer = "au2", driver = "ESRI Shapefile", overwrite_layer = TRUE)

# SAVE, COMMIT, AND PUSH, THEN

au <- readOGR(dsn = paste0(fl, "au2.shp"), layer = "au2")


au <- merge(au2, dat, by.x = "IDlong", 
                        by.y = "IDlong")
names(au)
...
for(i in 1:3){
 ...
 box(lty = 1, col = "grey")
  mtext("% area cultivated (5yr average)", side = 3, outer = TRUE)
  if ( i == 1){
...
# Majority visible from area of high traffic
layout(matrix(c(1,2,1,3), 2, 2, byrow = T), widths = c(1.4,1))
par(mar = c(0, 0, 0, 0), oma = c(0, 0, 3, 0))
for(i in 1:3){
  reg <- au[au$region == i, ]

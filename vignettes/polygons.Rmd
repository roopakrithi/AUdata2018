...
#table(au$IDlong)
#autest <- aggregate(x = au, by = list(au$IDlong), mean)
#auT <- unionSpatialPolygons(au, au$IDlong)
#?gUnaryUnion
#gIsValid(au)
## there's some overlapping polygons here, and that's a problem. how to resolve?
...
for(i in 1:3){
  ...
  box(lty = 1, col = "grey")
  mtext("Majority isible from settlement or high-traffic area", side = 3, outer = TRUE)
  if ( i == 1){
    legend("bottomright", c("y", "n"), fill = c("blue","white"), bty = "n")
  }
 }
